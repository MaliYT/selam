...
exports.run = async(client, message, args) => {
  ...
  if(!bymayfe) return message.channel.send("Bir seçenek belirtin (ayarla - kapat)");
    ...
    .addField('Sadece herhangi bir sunucudan mesaj gönderebilirim.:x: ')
 ...
 let rol = message.mentions.roles.first() || message.guild.roles.get(args.join(' '))
  let newRole;
  let tworole;
  if (!rol) return message.channel.send(':x: Otorol ayarlamanız için bir rol etiketlemeniz gerek. `c!otorol ayarla @Üye #kanal`')
  ...
  if (!otorolkanal) return message.channel.send(':x: Otorol ayarlamanız için bir rol etiketlemeniz gerek. `c!otorol ayarla @Üye #kanal`')
