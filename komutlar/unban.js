...
module.exports.run = async (client, message, args) => {
  ...
  const userError = new Discord.RichEmbed()
    .setColor('#ed455a')
      .setTitle('• Hata: 02 •')
        .setDescription('```Yasağı kaldırmak için bir kullanıcı kimliği girmelisiniz c!unban İD```')
 ...
 // console.log(banList.map(s => s.users.id))
  ...
  var mbed = new Discord.RichEmbed()
  .setColor("GREEN")
  .setDescription(`<@!${user}> Adlı Kullanıcının Yasağı Başarıyla Kaldırıldı.`)
